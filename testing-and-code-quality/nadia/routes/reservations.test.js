...
beforeAll(() => {
	// replaces the unified router in routes/index.js with only the reservations router
...
describe('POST', () => {
	...
	it('should accept a valid reservation request', async () => {
		...
		expect(response.text).toContain('Thanks, your booking request #1349 is waiting to be confirmed. Updates will be sent to the email address you provided.')
	})
})
