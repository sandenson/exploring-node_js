...
router.get("/:itemId?", async (req, res, next) => {
  try {
    // Get all items
    const items = await CatalogServiceClient.getAll();

    let item = null;

    // If an itemId was provided, get that specific item
    if (req.params.itemId) {
      item = await CatalogServiceClient.getOne(req.params.itemId);
    }
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!req.body.itemId) {
      await CatalogServiceClient.create({ sku, name, price }, req.session.token);
    } else {
      // Update an existing item
      const itemData = { sku, name, price };
      await CatalogServiceClient.update(req.body.itemId, itemData, req.session.token);
    }
...
router.get("/delete/:itemId", async (req, res) => {
  try {
    // Remove the item
    await CatalogServiceClient.remove(req.params.itemId, req.session.token);
