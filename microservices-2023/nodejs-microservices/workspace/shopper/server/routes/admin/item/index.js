...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!req.body.itemId) {
      await CatalogServiceClient.create({ sku, name, price });
    } else {
      // Update an existing item
      const itemData = { sku, name, price };
      await CatalogServiceClient.update(req.body.itemId, itemData);
    }
...
router.get("/delete/:itemId", async (req, res) => {
  try {
    // Remove the item
    await CatalogServiceClient.remove(req.params.itemId);
