...
server.on("listening", () => {
  ...
  const register = async () => axios
    .put(`http://127.0.0.1:3080/register/${config.serviceName}/${config.serviceVersion}/${addr.port}`)
    .catch((err) => console.error(err))

  const unregister = async () => axios
    .delete(`http://127.0.0.1:3080/register/${config.serviceName}/${config.serviceVersion}/${addr.port}`)
    .catch((err) => console.error(err))
